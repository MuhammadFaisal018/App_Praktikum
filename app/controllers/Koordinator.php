...
class Koordinator extends Controller
{
    ...
    public function index()
    {
        ...
        $this->view('koordinator/index', $data);
        $this->view('templates/footer');
    }

    public function tambah()
    {
        $data['title'] = 'Tambah Koordinator';
        $data['wilayah'] = $this->model('WilayahModel')->getAllWilayah();

        $this->view('templates/header', $data);
        $this->view('templates/sidebar', $data);
        $this->view('koordinator/create', $data);
        $this->view('templates/footer');
    }

    public function simpanKoordinator()
    {
        if ($this->model('KoordinatorModel')->tambahKoordinator($_POST) > 0) {
    ...
    {
        $data['title'] = 'Detail Koordinator';
        $data['koordinator'] = $this->model('KoordinatorModel')->getKoordinatorById($id);
        $data['wilayah'] = $this->model('WilayahModel')->getAllWilayah();

        $this->view('templates/header', $data);
        $this->view('templates/sidebar', $data);
        $this->view('koordinator/edit', $data);
        $this->view('templates/footer');
    }

    public function updateKoordinator()
    {
        if ($this->model('KoordinatorModel')->updateDataKoordinator($_POST) > 0) {
    ...
    {
        if ($this->model('KoordinatorModel')->deleteKoordinator($id) > 0) {
    ...
    public function cari()
    {
        ...
        $data['koordinator'] = $this->model('KoordinatorModel')->cariKoordinator($data['key']);
    ...
    {
        $data['title'] = 'Data Laporan Koordinator';
        $data['koordinator'] = $this->model('KoordinatorModel')->getAllKoordinator();
        $this->view('koordinator/lihatlaporan', $data);
    }
    ...
    public function laporan()
    {
        ...
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(190, 7, 'LAPORAN DATA KOORDINATOR PARKIR', 0, 1, 'C');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(8, 6, 'No', 1);
        $pdf->Cell(27, 6, 'Nama Koordinator', 1);
        ...
        foreach ($data['koordinator'] as $row) {
            $pdf->Cell(8, 6, $row['id_id_koor'], 1, 0, 'C');
            $pdf->Cell(27, 6, $row['nama_koor'], 1);
            $pdf->Cell(38, 6, $row['tempat_tgl_lahir'], 1);
            $pdf->Cell(25, 6, $row['ktp'], 1);
        ...
        $pdf->Output('I', 'Laporan Koordinator.pdf', true);
    }
}
