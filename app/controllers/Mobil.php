...
class Mobil extends Controller
{
    ...
    public function cari()
    {
        ...
        $data['mobil'] = $this->model('MobilModel')->cariMobil($key);
        $data['key'] = $key;
        $this->view('templates/header', $data);
        $this->view('templates/sidebar', $data);
        $this->view('mobil/index', $data);
    ...
    public function laporan()
    {
        ...
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(190, 7, 'LAPORAN DATA MOBIL PARKIR', 0, 1, 'C');
        ...
        foreach ($data['mobil'] as $row) {
            $pdf->SetX(30);
            $pdf->Cell(8, 6, $row['id_mobil'], 1, 0, 'C');
        ...
        $pdf->Output('I', 'Laporan Mobil.pdf', true);
    }
}
